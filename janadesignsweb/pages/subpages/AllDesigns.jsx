...
const AllDesigns = () => {
  ...
  return (
    <>
    <ADnavbar/>
    <section className="all-designs-section">
      ...
      <div className="all-designs-wrap">
        

      <h2 className="all-designs-title">All Designs</h2>

      {Object.entries(designs).map(([type, items]) => (
        <div key={type} style={{ marginBottom: '40px' }}>
          <h3 style={{ color: '#2c4152', marginBottom: '20px' }}>{type}</h3>
          <div className="all-designs-grid">
            {items.map((design) => (
              <div
                className="all-design-card"
                key={design._id}
              >
                <div className="all-image-wrapper">
                  <img src={design.image} alt={design.name} />
                  <button
                    className="all-info-button"
                    onClick={() => navigate(`/design/${design._id}`)}
                  >
                    info
                  </button>
                </div>
                <div className="	all-design-name">{design.name}</div>
              </div>
            ))}
          </div>
        </div>
